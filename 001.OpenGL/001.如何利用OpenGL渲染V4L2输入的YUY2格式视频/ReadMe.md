...
>  * Y --- 亮度分量  U,V --- 色度分量
>
>  * 按照分量存储的连续性分为`planar`（YUV分别连续存储),`semi-planar`(Y分量连续,UV分量交叉存储)和`packed`（YUV分量交叉存储）
>
>  * 按照采样规则又分为`444`(每个Y对应一组UV),`422`(每两个Y共用一组UV)和`420`(每四个Y共用一组UV)
>
>  * 安卓默认相机里出来的raw数据格式为`NV21`(属于YUV420semi-planar，YYYYVU)或`YV12`(属于YUV420planar，YYYYVU)
>
>  * **而我们的V4L2里获取的数据是`YUY2`中的`YUYV`(属于YUV422packed，YUYV)`每四个字节表示两个像素点`，提问，`分辨率为WxH的图片，这个格式的字节数组有多长？`(这个问题的答案能帮助解决后续纹理颜色格式设置问题)**
